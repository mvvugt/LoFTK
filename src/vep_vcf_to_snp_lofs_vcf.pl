...
sub get_sample_names_and_CSQ_fields {
  ...
  shift @header_fields for 0 .. 8; #Skip the CHROM POS ID REF ALT QUAL FILTER INFO entries.
...
#    next unless $parsed_line->{filter} eq '.'; #Make it to accept any strings for filtered VCF files (\*)
