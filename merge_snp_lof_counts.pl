...
sub get_samples_string_from_header_line {
  ...
  my ( $snp_ID , $allele , $conseq , $gene_ID , $gene_symbol , $single_f , $double_f ) = ( shift @fields , shift @fields , shift @fields , shift @fields , shift @fields , shift @fields , shift @fields );
  return join "\t" , @fields;
}
...
sub main {
  ...
  my $header = join "\t" , ( "SNP_ID" , "Allele" , "Consequence" , "gene_ID" , "gene_symbol" , "heterozygous_LoF_frequency" , "homozygous_LoF_frequency" , ); #Start the header.
  for ( @contents ) {
    $header .= "\t" . ${ $_->[0] };
  }
  $output->say( $header );
